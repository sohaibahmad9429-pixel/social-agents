...
Production-ready request/response validation for Meta Marketing API v24.0

Updated December 2025 - Fully aligned with Meta Marketing API v24.0
...
# ============================================================================
# ENUMS - Meta Marketing API v24.0
# ============================================================================

class CampaignObjective(str, Enum):
    """
    Meta Ads Campaign Objectives - API v24.0 OUTCOME-based (ODAX)
    
    These are the 6 simplified campaign objectives as of v24.0:
...
class BidStrategy(str, Enum):
    """
    Bid strategy options - v24.0
    Note: TARGET_COST was deprecated in Marketing API v9
    """
...
class OptimizationGoal(str, Enum):
    """
    Optimization goal options - API v24.0
...
class DestinationType(str, Enum):
    """
    Destination type for ad sets - v24.0
...
class PlacementSoftOptOut(str, Enum):
    """
    Placement soft opt out options - v24.0 NEW
...
class MediaTypeAutomation(str, Enum):
    """
    Media type automation for Advantage+ Catalog Ads - v24.0
...
class TargetingSpec(BaseModel):
    """
    Complete targeting specification - v24.0
...
class PromotedObject(BaseModel):
    """
    Promoted object for conversion tracking - v24.0
...
class AttributionSpec(BaseModel):
    ...
    window_days: int  # 1, 7, 28
    weight: Optional[float] = 100


# ============================================================================
# LOOKALIKE AUDIENCE SPECS - v24.0 (Mandatory from Jan 2026)
# ============================================================================

class LookalikeSpec(BaseModel):
    """
    Lookalike audience specification - v24.0 MANDATORY from Jan 6, 2026
...
# ============================================================================
# CAMPAIGN SCHEMAS - v24.0
# ============================================================================

class CreateCampaignRequest(BaseModel):
    """
    Request to create a campaign - v24.0
    
    Supports 75% daily budget flexibility (weekly cap = 7x daily budget)
    """
    name: str = Field(..., min_length=1, max_length=255)
    objective: CampaignObjective
    status: Optional[CampaignStatus] = CampaignStatus.PAUSED
    special_ad_categories: Optional[List[SpecialAdCategory]] = None
    buying_type: Optional[BuyingType] = BuyingType.AUCTION
    
    # Budget - either at campaign level (CBO) or ad set level
    budget_type: Optional[str] = None  # 'daily' or 'lifetime'
    budget_amount: Optional[float] = None  # In account currency
    
    # Bid strategy
    bid_strategy: Optional[BidStrategy] = None
    
    # Campaign Budget Optimization (CBO)
    is_campaign_budget_optimization: Optional[bool] = False
    
    # Schedule
    start_time: Optional[str] = None  # ISO 8601 format
    end_time: Optional[str] = None
    
    # Promoted object (required for iOS 14+ app campaigns)
...
class CampaignInsights(BaseModel):
    """Campaign performance insights - v24.0 metrics"""
...
class CreateAdSetRequest(BaseModel):
    """
    Request to create an ad set - v24.0
    ...
    # v24.0 NEW: Ad Set Budget Sharing
    ...
    # v24.0 NEW: Placement soft opt out
...
class ObjectStorySpec(BaseModel):
    """
    Object story specification for ad creatives - v24.0
    Creates an unpublished page post for the ad
    """
    page_id: str
    instagram_actor_id: Optional[str] = None  # Deprecated, use instagram_user_id
...
class AdCreative(BaseModel):
    """
    Ad creative specification - v24.0
    
    Note: For v24.0, prefer using object_story_spec for new creatives
    """
    title: Optional[str] = Field(None, max_length=40)
    ...
    # Object Story Spec (preferred for v24.0)
    object_story_spec: Optional[ObjectStorySpec] = None
    
    # v24.0: Media type automation for Advantage+ Catalog Ads
...
# ============================================================================
# AUDIENCE SCHEMAS - v24.0
# ============================================================================

class CreateCustomAudienceRequest(BaseModel):
    """Request to create a custom audience - v24.0"""
    name: str = Field(..., min_length=1, max_length=255)
    description: Optional[str] = None
...
class CreateLookalikeRequest(BaseModel):
    """
    Request to create a lookalike audience - v24.0
...
class InsightsRequest(BaseModel):
    """Request for insights data"""
    date_preset: Optional[DatePreset] = DatePreset.LAST_7D
    time_range: Optional[Dict[str, str]] = None  # since, until
